...
# Change the $modelfile to the model file you trained on your data.
$modelfile="$CNFHOME/data/model.p0-900.1561189";
...
$jobtmpdir=`$CNFHOME/bin/mktemp -d --tmpdir=./ --suffix=.$seqPrefix`;
#$jobtmpdir=`mktemp -d -p ./.$seqPrefix`; # work only on cruncher , due to the mktemp 
chomp($jobtmpdir);
...
$cmd="$CNFHOME/bin/bcnf_mpitp CONF $CNFHOME/data/CNF.ax.norm.conf ACT null PREDICT1 1 FEATMASK 0 ALLRST all.rst.csv BIEN 1 TRAIN $FNTMPNULL TEST $FNTINPUT RESULT $FNTMPRST RESUME $modelfile &> $jobtmpdir/cnfsseight.$jobid.stderr";
`$cmd`;
