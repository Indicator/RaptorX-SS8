...
void Minimizer::ApproximateGradient (vector<double> &g, 
  ...
  //  for (int i = g.size()-20; i < (int) g.size(); i++){
    for (int i = 0; i < 40; i++){
    x_copy[i] += EPSILON;
...
if (proc_id==0)
{
	...
	while (true){
        ...
        if (proc_id==0)
        {
          time(&end);
          double dif = difftime (end,start);
          cerr << "ComputeGradient " << dif << " seconds." << endl;
          time(&start);
        }
        ...
        if(GRADIENTCHECK)
        {
          ...
          for (int i = 0; i < 40; i++){//g[k%M].size(); i++){
            char buf[100];
            for (int j =5; j <= 5; j++)
            {
              sprintf(buf,"%.4f\t%.4f\t%.4f", g[k%M][i],aa[j][i],1-g[k%M][i]/aa[j][i]);
              cerr <<buf<<"\n";
            }
...
if (proc_id==0)
{
	time(&end);
	double dif = difftime (end,start);
	cerr << "ComputeHessianDiagnal " << dif << " seconds." << endl;
	time(&start);
}
*/
...
if (proc_id==0)
{
	time(&end);
	double dif = difftime (end,start);
	cerr << "STEP THREE: Update iterates " << dif << " seconds." << endl;
	time(&start);
}
*/
...
if (proc_id==0)
{
	time(&end);
	double dif = difftime (end,start);
	cerr << "STEP FOUR: Compute new search direction " << dif << " seconds." << endl;
	time(&start);
}
*/
...
if (proc_id==0)
{
	time(&end);
	double dif = difftime (end,start);
	cerr << "LineSearch " << dif << " seconds." << endl;
	time(&start);
}
